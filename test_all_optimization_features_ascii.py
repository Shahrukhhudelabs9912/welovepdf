    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'application/pdf' in content_type:
                print("[OK] Fix Scanned PDF endpoint working correctly")
                return True
            else:
        ...
        print(f"[ERROR] Exception during fix-scanned-pdf test: {e}")
        return False
...
def test_optimize_pdf():
    ...
    try:
        ...
        if response.status_code == 200:
            content_type = response.headers.get('Content-Type', '')
            if 'application/pdf' in content_type:
                print("[OK] Optimize PDF endpoint working correctly")
                return True
            else:
                print(f"[ERROR] Unexpected content type: {content_type}")
                return False
        ...
        print(f"[ERROR] Exception during optimize-pdf test: {e}")
        return False
...
def test_prepare_print_pdf():
    ...
    try:
        ...
        if response.status_code == 200:
            content_type = response.headers.get('Content-Type', '')
            if 'application/pdf' in content_type:
                print("[OK] Prepare Print PDF endpoint working correctly")
                return True
            else:
                print(f"[ERROR] Unexpected content type: {content_type}")
                return False
