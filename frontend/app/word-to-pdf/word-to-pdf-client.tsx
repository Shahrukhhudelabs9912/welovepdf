      ...
      <div className="space-y-8">
        <div className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-6 shadow-lg">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h3 className="text-xl font-semibold">{t("upload_title")}</h3>
              <p className="text-gray-600 dark:text-gray-400">{t("upload_description")}</p>
            </div>
          </div>
                ...
                <div>
                  ...
                  {progress > 0 && (
                    <div className="mt-2">
                      ...
                      <p className="text-xs text-gray-500 mt-1">{t("progress_percent", { progress: progress.toFixed(0) })}</p>
                    </div>
                  )}
                </div>
              </div>
          ...
          {conversionStatus === 'success' && !isLoading && (
            <div className="mt-6 p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-medium">{t("conversion_success")}</p>
                  <p className="text-sm text-gray-600 dark:text-gray-400">{tp("download_auto")}</p>
                </div>
              </div>
            </div>
          )}
          ...
          {conversionStatus === 'error' && !isLoading && (
            <div className="mt-6 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-medium">{t("conversion_failed")}</p>
                  <p className="text-sm text-gray-600 dark:text-gray-400">{error || tp("try_again")}</p>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="p-4 border border-gray-200 dark:border-gray-800 rounded-lg">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-medium">{t("feature_preserve")}</span>
              </div>
              <p className="text-sm text-gray-600 dark:text-gray-400">{t("feature_preserve_desc_short")}</p>
            </div>
            <div className="p-4 border border-gray-200 dark:border-gray-800 rounded-lg">
              <div className="flex items-center gap-2 mb-2">
                <Sparkles className="h-4 w-4 text-blue-500" />
                <span className="font-medium">{t("feature_formats")}</span>
              </div>
              <p className="text-sm text-gray-600 dark:text-gray-400">{t("feature_formats_desc_short")}</p>
            </div>
            <div className="p-4 border border-gray-200 dark:border-gray-800 rounded-lg">
              <div className="flex items-center gap-2 mb-2">
                <Sparkles className="h-4 w-4 text-purple-500" />
                <span className="font-medium">{t("feature_batch")}</span>
              </div>
              <p className="text-sm text-gray-600 dark:text-gray-400">{t("feature_batch_desc_short")}</p>
            </div>
          </div>
        ...
        <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl p-6">
          <h3 className="text-lg font-semibold mb-3">💡 {t("why_convert_title")}</h3>
          <ul className="space-y-2 text-gray-700 dark:text-gray-300">
            <li className="flex items-start gap-2">
              <span className="text-green-500">•</span>
              <span dangerouslySetInnerHTML={{ __html: t.raw("why_convert_universal") }} />
            </li>
            <li className="flex items-start gap-2">
              <span className="text-green-500">•</span>
              <span dangerouslySetInnerHTML={{ __html: t.raw("why_convert_professional") }} />
            </li>
            <li className="flex items-start gap-2">
              <span className="text-green-500">•</span>
              <span dangerouslySetInnerHTML={{ __html: t.raw("why_convert_optimization") }} />
            </li>
            <li className="flex items-start gap-2">
              <span className="text-green-500">•</span>
              <span dangerouslySetInnerHTML={{ __html: t.raw("why_convert_security") }} />
            </li>
          </ul>
        </div>
      </div>
